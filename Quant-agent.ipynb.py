...
# MAGIC %pip install streamlit plotly fpdf
...
# NOTE: This assumes you're in Databricks and `spark` is available.
# If running outside Databricks, you'll need to set up a Spark/SQL client.
...
def prettify_flag(flag: str) -> str:
    ...
    # optional nicer mappings
    txt = txt.replace("extreme debt", "extreme debt")
    txt = txt.strip()
    return txt


# ============================================
# 1. LLM backend: call meta-llama via ai_query
# ============================================
...
"""
    ...
    def generate_screener_sql(self, nl_query: str) -> str:
        ...
        sql_text = call_llm_api(prompt).strip()
...
"""
        explanation = call_llm_api(expl_prompt)
...
def mean_variance_optimize(returns: pd.DataFrame) -> pd.Series:
    ...
    For a proper Markowitz or CVaR model, you'd use cvxpy, but for a hackathon this is ok.
    """
    cov = returns.cov()
    diag = np.diag(cov)
    inv_var = 1.0 / diag
...
def memo_to_pdf(memo_text: str, filename: str = "investment_memo.pdf") -> bytes:
    pdf = FPDF()
...
run_single = st.sidebar.button("Analyze Ticker")
...
with tab1:
    ...
    if run_single:
        ...
        st.info("Use the sidebar to set Ticker & Question, then click **Analyze Ticker**.")
...
with tab3:
    ...
    if run_single:
        try:
            ...
            st.write(f"Using spot={spot}, vol_annual={vol}%, factor-expected return≈{_round_or_none(exp_ret,2)}%")
